...
\- 目前尚不清楚该论文的解决方案如何处理他们对“自适应 r”的贡献。第 3.3 节描述了一个字典映射到完美值或 r、pipe。par. 和全对多算法。这是使用试验预先构建的。但是，这里甚至被定义为审判吗？一切都是离线计算的吗？这与r的“自适应”有什么关系？从文本中可以看出，似乎选择了最好的 r，我们无法深入了解它如何随时间变化。评估部分严重缺乏对迭代或层函数中 r 的一些评估。 
...
在图 3 中，根据第 2.1 节，容量系数必须大于或等于 1 时，容量系数从 0.5 开始。 - 作者声称切换并行性是“零成本”，但实际上，根据 big-O 表示法，它是恒定成本（第 3.1.2 节）。 - 表 4 使用 r，但 r 只是在文本后面定义，而表 3 中没有。 - 图 7 的标题和图之间的 GPU 数量不一致。不清楚表 5 中的步骤是什么，是迭代还是批处理，还是...？ - 图 14 中显示的 Tutel bot 的纯计算开销，而不是 Baseline（尽管在查看图 15 时这有点多余？
