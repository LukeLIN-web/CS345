...
https://github.com/UofT-EcoSystem/hotline

以后的展示, 解释 topic是什么? 为什么重要? What is previous work?  之前工作的缺点是啥? 我们工作的优点是啥? 为什么有这些优点? 

作为审稿人，你什么情况下会选择拒稿？ - 戴晓天的回答 - 知乎 https://www.zhihu.com/question/529489400/answer/2462037699

引用足够多的以往的工作，并对该问题目前的发展水平有概括性的介绍。过去的工作所没有解决的问题.
...
作者是否将实验的条件设置写清楚了，实验的对照是否合理，实验结果的图表是否清晰、规范，对于实验结果的分析是否充分、合理. 是否缺少评估. (但是我也不知道要评估啥啊? )

提出的假设要多思考,是不是错的

problem is well known, not new problem.not ground -breaking

not exciting.  

## [Reliable Recommendation Model Training via Erasure Coding](https://eurosys23shadow.hotcrp.com/paper/1)

Liangliang Xu

### 论文摘要

本文提出了 ECRec，一种使用纠删码为 DLRM 训练系统中的推荐模型训练提供容错的技术。本文发现，随着模型大小的增加趋势，广泛使用的检查点会消耗大量网络带宽并导致大量存储占用空间。

为了解决这些挑战，ECRec 编码嵌入表及其优化器状态以降低内存开销，并复制 NN 参数及其优化器状态以降低网络带宽开销。ECRec 利用差异传播来正确更新冗余参数。此外，它使用粒度锁定实现无暂停恢复，使用 2PC 实现一致性。

评估表明，ECRec 减少了大型 DLRM 的训练时间开销，并且具有更快的恢复速度。

### 重大弱点

1. 缺乏新颖性：ECRec 直接在 DLRM 训练系统中部署纠删码，只是利用了纠删码的固有操作。大多数技术与现有作品相似。
2. 评价没有说服力。a。比较检查点基线不是最先进的。b. 没有关于纠删码参数的敏感性评估。C。没有评估关于近似检查点的精度损失描述。

### 优势

1. Section 1和Section 2的写作条理清晰。
2. 清晰分析在 DRLM 培训系统中部署 EC 的动机和挑战。

### 给作者的评论

这项工作结合了 DLRM 特性和擦除编码，以在 DLRM 训练中提供容错。我喜欢阅读对挑战的分析。但令我惊讶的是，大多数解决这些挑战的方法都是提议的工作或现有方法的简单实践。就我而言，这项工作有以下局限性：

1. ECRec 的关键见解是使用混合方法在 DLRM 训练中提供冗余，其中纠删码嵌入表并复制 NN 参数。这个想法被广泛用于纠删码内存键值存储。此外，通过差异传播更新冗余参数和2PC一致性也是擦除编码系统中广泛采用的方式，没有区别。(太trivial没啥新意)
2. 评估提供了 ECRec 和传统检查点之间的详细比较，但没有将 ECRec 与最先进的检查点方法进行比较。所以我怀疑ECRec的性能。
3. Checkpoint 似乎能够提供更高的可靠性，即如果两个或更多服务器发生故障，系统可以重置到最近的检查点并从该点重新开始训练。但是论文只给出了一个奇偶校验的ECRec的评估结果。当 ECRec 和检查点的可靠性不同时，比较它们之间的吞吐量和延迟是否合理。论文自始至终认为单一故障是数据中心最常见的故障场景，随着模型规模的增加，集群中的节点数量也会增加。所以一个条带中只有一个奇偶校验可能不满足容错的要求。(假设不一定成立)
4. 该论文一直强调近似检查点可能会损失原始 DLRM 的准确性。但是，没有关于近似检查点精度降低的评估。(假设不一定成立)
5. 该论文指出，ECRec 在恢复速度方面比检查点有巨大的改进。然而，checkpointing 数据由持久化设备存储在 HDFS 后端，ECRec 将冗余数据保存在内存中。因此，很难确定改进是来自无磁盘访问还是 ECRec 的好处。( 没有公平变量)

大家都说这个论文evlaution 特别好.

https://mlsys23ae.hotcrp.com/ 

## 会议

PC 会开个会, 然后一篇一篇过. 大家都可以看到.  各个审稿人就发表自己的看法. 

我发现和运气也有很大关系, 有的就两个审稿人, 别人都不审, 他们不太懂, 然后就accepted了. 有的遇到7个审稿人.
...
Fast and Scalable Mixture-of-Expert Inference论文好, 就新网络, 对模型调整一下.

Changho Hwang 的导师是kyoungsoo park kaist,他也是ark的一作. 

maoyang 是系统组长,  Yongqiang Xiong是msra网络组长.

megablocks 是专家数大于 gpu 数。tutel是gpu大于专家数.

 非常新, 第一个展示adaptive MOE的论文.  

优点就是

1. 可以用很多GPU. figure14, 2048gpu的时候就很快, 

缺点: 

只搜索了EP-degree，搜索空间有限。

\- 目前尚不清楚该论文的解决方案如何处理他们对“自适应 r”的贡献。第 3.3 节描述了一个字典映射到完美值或 r、pipe。par. 和全对多算法。



这是使用试验预先构建的。但是，这里甚至被定义为trial吗？一切都是离线计算的吗？这与r的“自适应”有什么关系？从文本中可以看出，似乎选择了最好的 r，我们无法深入了解它如何随时间变化。评估部分严重缺乏对迭代或层函数中 r 的一些评估。 

图 11 没有上下文，作者也没有讨论为什么会突然下降到大约 256 个 GPU。它就在那里。 - 自适应流水线似乎无处不在：目前尚不清楚这个“基线解决方案（流水线度 1）”是什么。在没有任何离散分析的情况下对 243 个模型进行平均改进也不是一个好主意。表6b）中的最坏情况似乎是Tutel的角度来看最好的情况，而从基线的角度来看是最坏的情况。

对 Tutel 和 Fairseq 之间的细分比较的评估很好，可以很好地了解大多数改进的地方。与图 14 中的增量计算相同。

 - 其他要点 - 

图 5 显示了最佳管道策略，但不清楚它们是如何获得的，或者它们是什么。 (就是做实验获得.  就是有各种configuration可以达到. )

表 2 中的数字似乎表明，当 GPU 数量为 256 个时，还有另一个因素在起作用，但没有显示出来。 - 

在图 3 中，根据第 2.1 节，容量系数必须大于或等于 1 时，容量系数从 0.5 开始。 - 作者声称切换并行性是“零成本”，但实际上，根据 big-O 表示法，它是恒定成本（第 3.1.2 节）。 - 表 4 使用 r，但 r 只是在文本后面定义，而表 3 中没有。 - 图 7 的标题和图之间的 GPU 数量不一致。不清楚表 5 中的步骤是什么，是迭代还是批处理，还是...？ - 

图 14 中显示的 Tutel bot 的纯计算开销，而不是 Baseline（尽管在查看图 15 时这有点多余？

14 为什么pure是最快的, 时间最短, 别的反而时间更长?  看懂了, 他把时间break down了, 分开了计算时间和通讯时间.



第二个反对者 

在我看来，实验部分不令人满意： - 他们提出的方法有一个关键特征，即 R 可以动态变化，但从未显示过如何随时间变化 -3.3   没有讲how often. 

这个应该是最大的问题, 好像就是查表. 3.3 好像就是一个黑盒方法.

表 6：不清楚值是什么，因为没有说明什么是基线（只有管道度数=1），此外，“最坏情况”的改进高于平均情况的改进。最坏情况是否是指基线？但是，我们对基线不感兴趣，而是对方法感兴趣，因此最坏的情况应该参考方法。此外，考虑到它是 243 种不同配置的平均值，应该对不确定性进行一些量化（STD 或等效的东西）。

关于评估的其他（不是那么）小事： 

第 5.1.1 节：据说实验是在 1 个节点（即 8 个 GPU）上进行的，但后来他们说他们使用了 64 个 GPU.有吗? 没看到. 

图 12 中的值是如何计算的？ - 图 3（这可能是由于我的知识有限）：他们用 f=0.5 进行实验，但在公式1 他们说“f=1 is the minimum value indicating the most even token distribution.”  .   我觉得也没事, 就测一下no even 分布的时候.
